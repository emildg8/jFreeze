...
export function OrdersView() {
  ...
  const filtered = useMemo(() => {
    ...
    return orders.filter((o) => {
      ...
      if (!q) return true;
      const hay = [getStoreLabel(o.storeId), ...o.items.map((i) => i.name)]
        .join(" ")
        .toLowerCase();
  ...
  return (
    <Screen className={orders.length > 0 ? "" : "pb-4"}>
      <PageHeader
        title="История заказов"
        description="Чеки, почта и демо — всё попадает сюда"
      ...
      {!loading && orders.length === 0 && (
        <UserTip id="ofd-scan-hint" title="Совет по QR">
          QR на чеке обычно внизу. Наведите камеру или вставьте строку вручную.
        </UserTip>
      )}
      ...
      {!loading && orders.length === 0 && (
        <EmptyState
          ...
          description="Нажмите «Сканировать QR камерой» выше или загрузите демо."
          action={
            ...
            <Button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  setMessage("Последний заказ повторён");
                  await refreshCart();
                  await load();
                } catch (e) {
                  setError(
